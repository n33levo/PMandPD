...
\end{tabular}
\vspace{1.4cm}
{\itshape Both authors contributed equally to this work.}
\vspace{1.4cm}
...
The paper tests two core predictions using the Dominick's Finer Foods scanner dataset, which contains 3.24~million store-week-UPC observations spanning 566~laundry-detergent products across 93~stores and 396~weeks.
Regressions of per-unit price on log package size yield a coefficient of $-3.00$ (SE~$= 0.37$, $p < 0.001$), confirming that larger packages carry significantly lower per-unit prices. This is consistent with size-based surplus extraction.
...
Third, it tests two core predictions using the Dominick's Finer Foods scanner dataset: (i)~per-unit prices decrease with package size, consistent with size-based price discrimination, and (ii)~per-unit prices vary systematically across stores, consistent with location-level demand segmentation.
...
On the empirical side, it provides a rigorous analysis of over 3.24~million scanner observations documenting the patterns predicted by the model: a steep size--price gradient, significant cross-store dispersion, and brand-level heterogeneity in per-unit pricing.
...
Appendix~\ref{app:welfare} provides a formal welfare comparison, and Appendix~\ref{app:robustness} considers asymmetric markups.
...
\subsection{Package size, packaging, and channel-specific SKUs}
...
Robinson--Patman enforcement has oscillated between aggressive periods and decades of relative dormancy as antitrust policy shifted toward a consumer-welfare standard.\footnote{Joshua~D. Wright and Douglas~H. Ginsburg, ``Stop Making Sense: Reviving the Robinson-Patman Act,'' George Mason University Law \& Economics Research Paper (2007).}
...
Recent policy discussions suggest renewed interest in Robinson--Patman enforcement, particularly given rising concentration in retail and wholesale markets.\footnote{Winston \& Strawn LLP, ``Price Discrimination Claims Under the Resurging Robinson-Patman Act,'' \textit{Competition Corner}, 2024.}
...
Early critiques argued that Robinson--Patman conflicts with the consumer-welfare standard and that its focus on protecting individual competitors can punish pro-competitive differential pricing.\footnote{Wright and Ginsburg, ``Stop Making Sense.''}
...
Yonezawa, G\'{o}mez, and Richards (2019) find that wholesale prices diverge across retailers despite Robinson--Patman constraints, but do not model the product-modification mechanism through which firms implement this divergence. The present paper fills that gap.\footnote{Koichi Yonezawa, Miguel~I.\ G\'{o}mez, and Timothy~J.\ Richards, ``The Robinson--Patman Act and Vertical Relationships,'' \textit{American Journal of Agricultural Economics} (2019), \url{https://doi.org/10.1093/ajae/aaz049}. Accessed March 2026.}
This paper contributes by providing a tractable model in which product modification is an explicit decision variable that interacts with Robinson--Patman constraints. It also tests the model's predictions against large-scale scanner data.
...
If $b_1 \ne b_2$ and $k = L = 0$, then $\Pi^D(0) > \Pi^U$.
\end{proposition}
...
Figure~\ref{fig:scatter_channel} presents the same scatter but distinguishes standard SKUs from channel-specific SKUs. Products in the largest 5\% of package sizes are marked as channel-specific (square markers); this top-5\% threshold is a proxy, since the Dominick's data cover a single supermarket chain and true channel-exclusive SKUs are not labelled directly. These bulk-size packages cluster in the bottom-right of the figure: low per-unit prices and large sizes. This pattern is consistent with the model's prediction that size-based product modification allows manufacturers to serve more elastic, bulk-buying segments at lower per-unit prices.

\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{fig3_scatter_channel.png}
\caption{Package size and per-unit price, distinguishing standard SKUs (circles) from channel-specific large-format SKUs (squares, defined as the top 5\% of package sizes---a proxy for club-format availability). The dashed line shows the overall log-linear fit. \textit{Source:} Author's calculations from Dominick's scanner data.}
...
This cross-store dispersion within a \textit{single retail chain} is consistent with location-level demand segmentation and zone pricing.
...
This large, precisely estimated coefficient confirms that the size--price schedule is steeply declining. It is consistent with the quantity-discount logic of nonlinear pricing and with the model's prediction that size differentiation enables effective price discrimination.
...
The remaining within-brand size gradient is large and highly significant. This confirms that size-based pricing operates above and beyond brand effects.
...
Table~\ref{tab:robustness} re-estimates the full model under two alternative clustering schemes. The point estimates on $\ln(\text{Size})$ and the sale indicator are identical across all three columns, since clustering affects only the covariance estimator. Under store-level clustering (93 clusters), the standard errors are smaller than under UPC-level clustering (566 clusters). This is expected: with fewer clusters, the finite-sample small-cluster correction matters less and the asymptotic approximation is somewhat more aggressive. Under two-way clustering by UPC and store using the Cameron, Gelbach, and Miller (2011) formula, the standard errors are close to those from UPC-only clustering, confirming that UPC-level dependence is the dominant source of within-sample correlation. All main coefficients remain significant at the 1-percent level across all three specifications.

\input{robustness_table.tex}

\subsection{Interpretation and limitations}

The empirical results are consistent with the two central predictions of the model.
...
Even within a single chain, where wholesale contracts are presumably uniform, local demand conditions, competitive environments, or zone-pricing policies generate meaningful variation in per-unit prices across locations.
...
In particular, package size is a manufacturer's strategic choice, not a random product feature. Larger detergent bottles may contain more concentrated formula, independently justifying lower per-unit prices. The brand fixed effects in specification~(3) address quality differences at the brand level. Within a brand, the same core formulation typically appears across multiple package sizes, so the residual within-brand size variation is more credibly driven by deliberate nonlinear pricing. Formally establishing a causal effect would require an instrument for package-size choices; candidate instruments include store-level shelf-space constraints or wholesale-distribution logistics that determine which sizes each location stocks.
Fourth, Table~\ref{tab:robustness} confirms that the main results are robust to alternative clustering by store and to two-way clustering by UPC and store via the Cameron, Gelbach, and Miller (2011) formula. Point estimates are unchanged; standard errors under the two-way scheme are close to the UPC-clustered baseline.

Despite these limitations, the descriptive patterns are strikingly consistent with the model and with the institutional accounts of how manufacturers use package-size differentiation to segment channels under Robinson--Patman.
...
The Dominick's data confirm this: Tide alone appears in sizes from 50~oz to over 200~oz, with per-unit prices ranging from roughly 5 to 12~\textcent/oz.
...
Figure~\ref{fig:retailer_box} presents an illustrative simulation of cross-channel price differences, calibrated to the Dominick's data and documented retail margins across grocery, mass-merchant, and club-store formats. This figure is a simulation, not a direct measurement, since the Dominick's data cover one supermarket chain. It illustrates the order of magnitude of cross-channel dispersion that motivates the model. Per-unit prices are highest in the grocery channel and lowest in club stores, with a gap of approximately 5~\textcent/oz. In the model, this gap equals $w_1^* - w_2^*$ plus downstream markup differences, and it is precisely the gain from closing this gap that drives the manufacturer's incentive to create channel-specific variants.
...
Figure~\ref{fig:welfare} illustrates the welfare decomposition under uniform pricing and under two-price discrimination for two demand segments. Under discrimination, the deadweight-loss triangle shrinks in the elastic segment (served at a lower price) but may expand in the inelastic segment. The net welfare effect depends on total output, as formalized in the appendix.
...
Empirical analysis of the Dominick's Finer Foods scanner dataset, covering 3.24~million observations across 566~laundry-detergent UPCs, 93~stores, and 396~weeks, confirms two key predictions.
...
\end{equation}
When $k = 0$ and the demand intercepts are symmetric ($a_1 = a_2$, $m_1 = m_2$), discriminatory and uniform prices coincide only if $b_1 = b_2$. With $b_1 \ne b_2$, the output effect in~\eqref{eq:output_condition} can be positive, negative, or zero depending on the specific parameter values. This confirms that discrimination via product modification does not unambiguously raise or lower welfare, consistent with Schmalensee (1981) and Varian (1989).
...
\bibitem{GMU2007}
Wright, Joshua~D., and Douglas~H. Ginsburg.
``Stop Making Sense: Reviving the Robinson-Patman Act.''
George Mason University Law \& Economics Research Paper, 2007.
\url{https://www.law.gmu.edu/pubs/papers/2603}.
Accessed March 2026.
